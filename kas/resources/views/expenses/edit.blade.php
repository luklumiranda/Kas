...
<div class="container-fluid py-4">
    <div class="row">
        <div class="col-md-8 offset-md-2">
            <h1 class="h3 font-weight-bold mb-4"><i class="fas fa-edit mr-2"></i>Edit Pengeluaran</h1>
                    ...
                    <form action="{{ route('expense.update', $expense) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="amount" id="amount" class="form-control @error('amount') is-invalid @enderror" 
                                step="1" min="0" value="{{ $expense->amount }}" required>
                            @error('amount')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            <label>Bukti yang Sudah Ada ({{ $expense->receipts->count() }} file)</label>
                            <div class="row">
                                @foreach($expense->receipts as $receipt)
                                <div class="col-md-3 mb-2">
                                    <small class="d-block">{{ Str::limit($receipt->file_name, 20) }}</small>
                                    <a href="{{ asset('storage/' . $receipt->file_path) }}" target="_blank" class="btn btn-sm btn-info" title="Download">
                        ...
                        <div class="form-group">
                            <label for="receipts">Tambah Bukti Baru (Opsional)</label>
                            <div class="custom-file">
                                <input type="file" name="receipts[]" id="receipts" class="custom-file-input" accept=".pdf,.jpg,.jpeg,.png" multiple>
                                <label class="custom-file-label" for="receipts">Pilih file tambahan</label>
                            </div>
                            <small class="form-text text-muted">Opsional - Jika ingin menambah bukti</small>
                        </div>

                        <div class="form-group">
                            <button type="submit" class="btn btn-primary mr-2"><i class="fas fa-save mr-1"></i> Update</button>
                            <a href="{{ route('expense.show', $expense) }}" class="btn btn-secondary"><i class="fas fa-times mr-1"></i> Batal</a>
                        </div>
                    </form>
                </div>
            </div>
